<h1 style="display: inline;"><img width="37" height="37" alt="image" src="https://github.com/user-attachments/assets/02f69ba8-9525-4e54-9d9e-987cd3aecb68" style="display:inline-block;"/>&nbsp;CSS Bookmark</h1>
Stop typing CSS and start doing more. This program will allow you to select CSS values from a drop down menu and copy them to the clip board. You can then just paste the values into your code.
<br><br>


<img width="1360" height="768" alt="image" src="https://github.com/user-attachments/assets/f3754dcb-4b44-4af3-a867-942577541f1b"/>




